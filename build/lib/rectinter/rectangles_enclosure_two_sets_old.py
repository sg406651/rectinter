...
def insert_query_inner(bst, A, node: int, query, set_idx):
    """
    ...
    if query[0] <= bst[node][0] and query[1] >= bst[node][1]:
        bst[node][4 + set_idx].append(query[2])
        A[query[2]][node] = len(bst[node][4 + set_idx]) - 1
        return
    ...
    if left_child is not None and query[0] <= bst[left_child][1] and query[1] >= bst[left_child][0]:
        insert_query_inner(bst, A, left_child, query, set_idx)
    if right_child is not None and query[0] <= bst[right_child][1] and query[1] >= bst[right_child][0]:
        insert_query_inner(bst, A, right_child, query, set_idx)


def insert_query(bst, A, query, set_idx):
    """
    ...
    set_idx: 0 or 1 for set A or B
    """
    insert_query_inner(bst, A, 0, query, set_idx)


def query_point_inner(bst, node, x, set_idx):
    """
    ...
    if x >= bst[node][0] and x <= bst[node][1]:
        res.extend(bst[node][4 + set_idx])
    left_child = bst[node][2]
    right_child = bst[node][3]

    if left_child is not None and x <= bst[left_child][1]:
        res.extend(query_point_inner(bst, left_child, x, set_idx))
    elif right_child is not None and x <= bst[right_child][1]:
        res.extend(query_point_inner(bst, right_child, x, set_idx))
    return res


def query_point(bst, x, set_idx):
    return query_point_inner(bst, 0, x, set_idx)


def delete_query(bst, A, query, set_idx):
    """
...
def rectangles_enclosure_two_sets(set_A, set_B):
    ...
    for i, rect in enumerate(set_A):
        ...
        x_axes.append((rect[0], 0, i, 0))
        x_axes.append((rect[2], 0, i, 2))
    ...
    for i, rect in enumerate(set_B):
        y_centroid = (rect[1] + rect[3]) / 2
        x_axes.append((rect[0], 1, i, 0))
        x_axes.append((rect[2], 1, i, 2))
    ...
    for rect in sorted_y:
        ...
        if set_idx == 0:
            if type == 0:
                insert_query(bst_x, A, (set_A[idx][0], set_A[idx][2], idx), set_idx)
            elif type == 2:
                delete_query(bst_x, A, (set_A[idx][0], set_A[idx][2], idx), set_idx)
            else:
                x = set_A[idx][0] / 2 + set_A[idx][2] / 2
                enclosure = query_point(bst_x, x, 1 - set_idx)
                for enc in enclosure:
                    set_A_enclosers[idx].append(enc)
                    set_B_enclosers[enc].append(idx)    
        else:
            if type == 0:
                insert_query(bst_x, B, (set_B[idx][0], set_B[idx][2], idx), set_idx)
            elif type == 2:
                delete_query(bst_x, B, (set_B[idx][0], set_B[idx][2], idx), set_idx)
            else:
